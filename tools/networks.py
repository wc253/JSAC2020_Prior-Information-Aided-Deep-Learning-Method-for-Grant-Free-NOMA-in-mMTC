...
def build_MPIAN(prob,T,initial_lambda=.1,untied=False):
    """
    ...
    xhat_ = eta( By_, lam0_,prob.s_)
    layers.append( ('LISTA T=1',xhat_, (lam0_,) ) )
    for t in range(1,T):
        lam_ = tf.Variable( initial_lambda,name='lam_{0}'.format(t) )
        xhat_ = eta( tf.matmul((np.identity(N) - tf.matmul(B_,A)),xhat_) + By_, lam_ ,prob.s_)
