...
def setup_training(layer_info,prob, trinit=1e-3,refinements=(.5,.1,.01),final_refine=None ):
    ...
    training_stages=[]
#    for name,xhat_,var_list in layer_info:   #train layer by layer
    for name,xhat_,var_list in [layer_info[len(layer_info)-1]]:# train end-to end
...
def do_training(training_stages,prob,savefile,ivl=10,maxit=100000,better_wait=50):
    """
    ...
    for name,xhat_,loss_,nmse_,train_,var_list in training_stages:
        ...
        for i in range(maxit+1):
            if i%ivl == 0:
                nmse = sess.run(nmse_,feed_dict={prob.y_:prob.yval,prob.x_:prob.xval,prob.s_:prob.sval})
                if np.isnan(nmse):
            ...
            y,x,s = prob(sess)
            sess.run(train_,feed_dict={prob.y_:y,prob.x_:x,prob.s_:s} )
        ...
        state['log'] = log
        if name[-4:]=='0.01':
            save_trainable_vars(sess,'LISTA_bg_giid'+str(name)+'.npz',**state)
